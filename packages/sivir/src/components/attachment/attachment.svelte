...
<div
	...
	ondrop={(event) => {
		...
		dragDepth = 0;
		dragging = false;
		if (!handled && hasFiles && !disabled && event.dataTransfer) addFiles(event.dataTransfer.files);
	}}
...
>
	...
	{#if dragging}
		...
		>
			<span
				class="flex items-center gap-2 rounded-full bg-card/90 px-3 py-1.5 text-sm font-medium shadow-[var(--elevation-control)]"
			>
