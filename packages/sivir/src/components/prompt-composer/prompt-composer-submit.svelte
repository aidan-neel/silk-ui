...
>
	<span class="relative grid size-4 place-items-center" aria-hidden="true">
		...
		<Square
			...
			strokeWidth={2.25}
			class={cn(
				'col-start-1 row-start-1 fill-current transition-[opacity,scale] [transition-duration:var(--motion-duration-panel)] ease-[var(--ease-out)] motion-reduce:transition-none',
				submitting ? 'scale-100 opacity-100' : 'scale-75 opacity-0'
			)}
		/>
	</span>
</Button>
