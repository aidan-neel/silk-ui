...
function overlayNestingDepth() {
    const parentDepth = getContext(OVERLAY_DEPTH) ?? 0;
...
function clearOverlayRootLayer(panel) {
    const root = panel.closest('[data-overlay-root]');
    if (!(root instanceof HTMLElement)) {
        return;
    }
...
function modalScrim(panel) {
    return panel
        .closest('[data-overlay-root]')
        ?.querySelector('[data-ui="modal-overlay"]');
}
function clearModalStackAttrs(panel) {
