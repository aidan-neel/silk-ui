...
describe('Attachment', () => {
	it('adds accepted files and removes the rendered item', async () => {
		...
		await userEvent.setup().click(screen.getByRole('button', { name: 'Remove release-notes.txt' }));
	...
	it('rejects an invalid file type with a typed code', async () => {
		...
		const input = container.querySelector<HTMLInputElement>('input[type="file"]')!;
		const file = new File(['report'], 'report.pdf', { type: 'application/pdf', lastModified: 2 });

		await chooseFiles(input, [file]);

		expect(screen.getByTestId('rejection-codes')).toHaveTextContent('report.pdf:file-invalid-type');
