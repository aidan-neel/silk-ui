...
describe('Markdown', () => {
	it('renders representative GFM structures', () => {
		...
		expect(screen.getByRole('heading', { level: 1, name: 'Release status' })).toBeInTheDocument();
		const table = within(screen.getByRole('region', { name: 'Markdown table' })).getByRole('table');
	...
	it('does not load remote or backslash-normalized images', () => {
		const { container } = render(Markdown, {
			props: {
				content: '![remote](https://example.com/pixel.gif)\n\n![escaped](/\\evil.example/pixel.gif)'
			}
		});
