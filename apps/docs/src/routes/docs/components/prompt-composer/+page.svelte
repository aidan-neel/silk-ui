...
<div data-docs-page class="flex flex-col gap-10">
	...
	<section id="usage" class="scroll-mt-20 flex flex-col gap-4">
		...
		<p class="text-sm text-foreground-muted">
			Bind the prompt value and handle submission on the root. The component awaits async handlers
			and shows its submitting state automatically.
		</p>
		<CodeBlock
...
<PromptComposer.Root bind:value onSubmit={sendPrompt}>
  ...
  <PromptComposer.Toolbar>
    ...
    <PromptComposer.Submit />
		...
		/>
		<p class="text-sm text-foreground-muted">
			By default, <kbd>Enter</kbd> submits and <kbd>Shift</kbd> + <kbd>Enter</kbd> inserts a new
			line. Set <code>submitOnEnter={false}</code> on <code>Input</code> when Enter should always create
			a new line.
		</p>
	</section>
