...
<div class="w-full max-w-2xl space-y-8">
	...
	<Message.Root from="assistant" status={retrying ? 'streaming' : 'error'}>
		<Message.Content>
			{#if retrying}
				<Markdown content="Reconnecting to the warehouse and rebuilding the query" streaming />
			{:else}
				<p>
					The warehouse connection closed before the query completed. Retry when the connection is
					available.
				</p>
			{/if}
		</Message.Content>
