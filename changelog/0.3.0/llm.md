...
`chrome` now carries `surfaceShadows`, `controlShadows`, and `dialogShadows` (default true; `shadows: false` still forces all off) plus `travelingHighlight: false`. Surface shadows own `--elevation-1` and `--elevation-float` (cards, selects, dropdowns, popovers, reorder rows); control shadows own `--elevation-control` and `--elevation-button-outline`; dialog shadows own `--elevation-modal`. The card inset surface composes `var(--elevation-1)` with its hairline, so turning surface shadows off leaves the hairline. `travelingHighlight: false` emits `--sivir-traveling-highlight: none`, which both hides `.sivir-item-highlight` via CSS and makes the `travelingHighlight` action bail at init — do not assume the action mounted.
